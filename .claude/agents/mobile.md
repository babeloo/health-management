...
- **测试要求**：在业务代码中抽象出"蓝牙数据解析器"，并为其编写独立的单元测试（Mock 硬件数据）
...
- API 请求统一封装,处理平台差异
- **逻辑隔离**：复杂算法（积分、生理指标转换）必须从 `.vue` 中提取到独立的 `.ts` 文件，以便使用 `vitest` 进行测试
...
- 用户友好的错误提示信息
- 关键操作记录日志用于问题排查
- **异步处理**：所有蓝牙和网络操作必须有完整的 try-catch 和用户友好提示
...
- 必要时附上相关官方文档链接
- 明确目标平台
- 提供完整可运行的 Vue 3 Composition API 代码
