...
- [x] 前后端联调测试（非 AI 功能） ✅ 完成于 2026-01-06
  ...
  - [x] 验证错误处理机制
  - **完成说明**：
    - 前端配置已正确指向 localhost:5000
    - 后端 NestJS 服务运行正常，所有非 AI 接口可用
    - Docker 基础设施服务（PostgreSQL, Redis, InfluxDB 等）运行正常
    - 代码通过 ESLint 和 TypeScript 检查
- [!] AI 功能联调测试 ⚠️ 阻塞中（独立任务）
  - [!] 测试 AI 聊天对话流程 ⚠️ 阻塞中
  - [!] 测试对话历史加载 ⚠️ 阻塞中
  - [!] 测试科普文章列表和详情 ⚠️ 阻塞中
  - **阻塞原因**：AI 服务启动失败
    - 本地编译问题：Python 依赖 `aiohttp==3.9.0` 需要 Visual C++ 14.0+
    - Docker 构建问题：网络速度过慢（下载 Python 基础镜像超过 10 分钟）
  - **影响范围**：仅影响 AI 相关功能（聊天、科普、辅助诊断），不影响其他核心功能
  - **预期解决时间**：待网络条件改善或用户安装编译工具后解决
  - **临时措施**：
    1. 已完成非 AI 功能的联调测试，确保核心业务可用
    2. AI 功能作为独立任务跟踪，不阻塞整体进度
    3. 建议解决方案：
       - 安装 Visual Studio Build Tools: https://visualstudio.microsoft.com/visual-cpp-build-tools/
       - 或等待网络条件改善后使用 Docker 构建
       - 或使用预编译的 wheel 包（需要 Python 3.11）
- [ ] E2E 测试验收 ⏸️ 待测试
  - [ ] 完整对话流程测试
  - [ ] 科普内容推荐测试
  - [ ] AI Agent 自然语言打卡测试
