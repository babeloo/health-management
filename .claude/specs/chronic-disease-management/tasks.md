...
> **最后更新**: 2025-12-22 11:15
> **总体进度**: 12.1% (33/273 任务)
> **当前阶段**: 第二阶段 - 后端核心服务（Week 2-6）
> **第一阶段进度**: 100% (23/23 已完成) ✅
> **第二阶段进度**: 20.8% (10/48 已完成) 🔄
> **状态**: 🔄 进行中（认证授权模块已完成，开始用户管理模块开发）
...
- [ ] 实现用户 CRUD 接口
  - [ ] 创建 UserModule、UserService、UserController
  - [ ] 实现获取用户信息接口（GET /api/v1/users/:id）
  - [ ] 实现更新用户信息接口（PUT /api/v1/users/:id）
  - [ ] 实现用户列表接口（GET /api/v1/users，支持分页和筛选）
  - [ ] 实现上传用户头像接口（POST /api/v1/users/:id/avatar）
- [ ] 集成文件存储服务
  - [ ] 安装 MinIO SDK（minio）
  - [ ] 创建 FileStorageService
  - [ ] 实现文件上传方法（uploadHealthDocument）
  - [ ] 实现带签名的文件访问 URL 生成
  - [ ] 配置文件大小限制（10MB）
- [ ] 编写用户模块测试
...
- [ ] 实现健康打卡接口
  ...
  - [ ] 实现打卡趋势分析接口（GET /api/v1/health/check-ins/:userId/trends）
  - [ ] 实现打卡日历视图接口（GET /api/v1/health/check-ins/:userId/calendar）
  - [ ] 添加唯一约束检查（每天每种类型只能打卡一次）
- [ ] 集成 InfluxDB 时序数据存储
...
- [ ] 实现积分交易接口
  - [ ] 创建 PointsModule、PointsService、PointsController
  - [ ] 实现获得积分接口（POST /api/v1/points/earn）
  - [ ] 实现消费积分接口（POST /api/v1/points/redeem）
  - [ ] 实现查询积分余额接口（GET /api/v1/points/balance/:userId）
  - [ ] 实现积分交易历史接口（GET /api/v1/points/transactions/:userId）
- [ ] 实现积分排行榜
  - [ ] 集成 Redis Sorted Set
  - [ ] 创建 CacheService（封装 Redis 操作）
  - [ ] 实现更新排行榜方法（updateLeaderboard）
  - [ ] 实现获取排行榜接口（GET /api/v1/points/leaderboard）
  - [ ] 实现批量获取用户信息方法（优化排行榜查询）
- [ ] 实现积分规则引擎
  - [ ] 定义积分规则配置（JSON 配置文件）
  - [ ] 实现积分计算服务（根据打卡类型计算积分）
  - [ ] 实现连续打卡奖励逻辑（7 天、30 天额外奖励）
  - [ ] 在打卡接口中自动触发积分发放
- [ ] 编写积分模块测试
  - [ ] 单元测试：PointsService 积分计算
  - [ ] 单元测试：连续打卡奖励逻辑
  - [ ] 集成测试：积分获得和消费
  - [ ] 集成测试：排行榜更新和查询
  - [ ] E2E 测试：打卡自动发放积分流程
